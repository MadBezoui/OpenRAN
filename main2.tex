...
% 2. Replace the placeholder Zenodo DOI once the archive is minted.
% 3. Verify all references, affiliations, funding statements,
%    author contributions, data, and numerical results before submission.
% ============================================================
...
finite-domain constraint optimization problem and analyze
non-integral fixed points of a relation-conditioned
product-of-experts update. In the smallest arbitration motif we prove
the existence of an exact fractional fixed point; in larger instances
we detect its operational counterpart, a stable continuous
configuration whose deterministic decoding still produces an invalid
or service-level-agreement-violating control decision. We then
introduce \method, a verified hybrid framework that combines
continuous marginal inference, explicit stall diagnostics, exact
repair of a conflict-induced subproblem, and an independent decision
verifier. The framework distinguishes direct, indirect, and implicit
conflicts and returns a control action only after validating all
registered hard constraints. The experimental protocol evaluates the
approach on reproducible O-RAN scenarios involving energy saving,
mobility robustness, load balancing, power control, and
radio-resource allocation. Comparisons include uncoordinated
execution, static-priority arbitration, continuous-only coordination,
and complete constraint programming. The results show that
continuous-only coordination stalls on approximately one quarter of
decision epochs and reaches only 74.7\% verified feasibility, whereas
both complete CP-SAT and \method\ attain 100\% verified feasibility.
On synthetic instances of this size the exact CP-SAT solver is itself
fast and complete (median 6~ms), so the continuous stage of \method\
adds latency (median 42~ms) rather than reducing it; the contribution
of \method\ is therefore not speed but the explicit stall diagnosis
and the independent verification guarantee, with exact repair confined
to a median core of two variables.
}
...
decision must belong to a finite set. This behaviour is well
understood in the wider inference literature: max-product belief
propagation is known to converge to the correct integral solution
only when the underlying linear-programming relaxation has no
fractional optimum, and can otherwise stall at a fractional vertex
\cite{park2015maxproduct,bayati2011bmatching}. The present work
transfers this observation to the specific setting of finite-domain
xApp arbitration.
...
when the equality $T(\bm{p})=\bm{p}$ holds exactly, and an
\emph{operational fractional stall} when it is only detected
numerically. The concept concerns the fixed points of the
implemented update map; it is not presented as a stationary point of
an unspecified global optimization objective.
...
\begin{enumerate}
    ...
    \item explicit fractional-stall detection;
...
\begin{itemize}
    ...
    \item We prove the existence of an exact fractional fixed point in
    the smallest xApp arbitration motif, and define an operational
    test that detects its numerically stable counterpart in larger
    instances, showing how both yield an infeasible decoded action.
    ...
    stability, and computational overhead, and we report honestly
    that on instances of the tested size the continuous stage adds
    latency relative to a complete solver.
\end{itemize}
...
Section~\ref{sec:analysis} analyzes fractional fixed points.
Section~\ref{sec:experiments} defines the experimental protocol.
...
If the constraint allows no valid tuples, the model is locally
infeasible. Numerical underflow during the marginalization is
prevented through log-sum-exp normalization.
...
global objective. The hyperparameters that govern $T$ and the
diagnostics of Section~\ref{sec:analysis}, together with the values
used in all experiments, are listed in Table~\ref{tab:hyperparams}.

\begin{table}[t]
\centering
\caption{Hyperparameters of the continuous update map and stall
diagnostics, with the values used in all experiments.}
\label{tab:hyperparams}
\begin{tabular}{llr}
\toprule
Symbol & Role & Value\\
\midrule
$\tau$ & Message temperature & $0.5$\\
$\varepsilon$ & Log/clip floor & $10^{-9}$\\
$\gamma_c$ & Local-utility weight & $0.0$\\
$\alpha$ & Inertia weight & $0.1$\\
$\beta$ & Polarization exponent & $2$\\
$\delta_{\rho}$ & Residual stall threshold & $10^{-4}$\\
$\delta_{\phi}$ & Fractionality threshold & $0.05$\\
$h$ & Repair-core expansion radius & $1$\\
& Max.\ continuous epochs & $50$\\
& Repair time limit & $50$~ms\\
\bottomrule
\end{tabular}
\end{table}
...
and operationally invalid state. An exact fractional fixed point, for
which $T(\bm{p})=\bm{p}$ holds analytically, is established only for
the motif of Section~\ref{sec:analysis}; throughout the empirical
results the quantity that is measured is the operational stall of the
definition above, and the two must not be conflated.
...
Proposition~\ref{prop:soundness} is a design guarantee of the
pipeline rather than a statement about the physical RAN: it does not
prove that the registered model perfectly represents the network.
Model validity and decision verification are separate properties.
...
At $\tau=1$, $\beta=1$, $\alpha=0$, and without clipping, the update
is
...
\begin{proposition}[Exact fractional xApp arbitration point]
\label{prop:motif}
...
Proposition~\ref{prop:motif} is the only place in this paper where an
\emph{exact} fractional fixed point is established analytically; in
larger instances we observe its operational counterpart under the
diagnostics of Section~\ref{sec:method}.
...
constants and batching effects. Relations that are more naturally
expressed intensionally (for example, an all-different relation over a
large domain) need not be materialized as explicit tuple tables; in
that case the per-epoch cost is governed by the specialized
propagator rather than by $|R_c|$.
...
evaluation reports the full distribution of $k$, rather than only its
mean.
...
\begin{description}
    ...
    \item[RQ3:] What control latency and computational overhead are
    introduced by continuous coordination, repair, and verification,
    relative to a complete solver?

    \item[RQ4:] How localized are the resulting conflicts, as measured
    by the repair-core size distribution?

    \item[RQ5:] How do conflict density and action-domain size,
    controlled through the fraction of symmetric separation relations,
    affect the stall rate?
\end{description}

We deliberately do not pose a network-utility research question:
because the synthetic action utilities are drawn independently of the
pseudo-physical KPMs (Section~\ref{sec:network-perf}), this simulator
cannot discriminate between coordination methods on throughput or
energy, and any such comparison would be uninformative. Network-level
utility is therefore left to a testbed study.

\subsection{Experimental platform}

The experiments are conducted using a synthetic Python-based O-RAN
continuous-arbitration simulator. The simulator models a
finite-domain xApp coordination problem, a continuous
relation-conditioned coordinator, an exact CP-SAT repair adapter
based on Google OR-Tools \cite{ortools}, an independent verifier, and
a pseudo-physical network model that maps coordinated actions to
transmission power, radio-resource fraction, and handover offsets in
order to derive SINR, throughput, energy, fairness, and
handover-failure indicators.

For reproducibility, the released artifact reports the following: the
Python version (3.11) and OR-Tools version used; the exact solver,
repair, and verifier implementations; the instance-generation
procedure and its seed hierarchy; the configuration of every
scenario; and the processor and memory of the evaluation host. All
instances are generated with a planted feasible solution, so that a
conflict-free decision is guaranteed to exist and any observed
infeasibility reflects a coordination failure rather than an
...
\caption{Verified feasibility and stall rates across arbitration
methods. \method\ repairs every detected stall and matches the
$100\%$ verified feasibility of the exact solver, whereas
continuous-only coordination stalls on $25\%$ of epochs.}
...
\caption{Cumulative distribution of decision latency. Complete CP-SAT
is the fastest method on instances of this size; the continuous
inference stage makes \method\ slower than CP-SAT, and
continuous-only coordination shares the latency profile of \method\
without its feasibility guarantee.}
\label{fig:latency}
\end{figure}

\subsection{Instance generation}

Each scenario corresponds to a distinct conflict regime, controlled
by the number of xApp action variables, the action-domain size, the
number of conflict relations, and the fraction of \emph{symmetric
separation} relations. A separation relation requires two xApp actions
to differ; such relations are the discrete counterpart of the
fractional-stall motif of Section~\ref{sec:analysis} and are the
primary source of stalls in the continuous coordinator. The regimes
range from $12$ action variables with a domain of size~$3$ (S1) to
$30$ action variables with a domain of size~$4$ and $32$ relations
(S5, the multi-xApp stress test). The underlying network uses $10$
cells and $100$ users with an offered traffic load of $0.6$ and
random-waypoint mobility at $3$~km/h.

A hidden feasible target assignment is planted in every instance, and
each generated relation is constructed so that the target satisfies
it. This guarantees that at least one conflict-free decision exists,
so a complete solver attains full feasibility and any residual
infeasibility is attributable to a coordination or decoding failure.
The generator seed hierarchy is fixed before the main evaluation; the
main experiment uses $30$ independent seeds per scenario. Because the
stall rate is a direct function of the symmetric-separation fraction,
we treat that fraction as an explicit sweep variable in
Section~\ref{sec:results} rather than as a fixed property of realistic
traffic; this is discussed further as a threat to external validity in
...
A generic QoS-aware exact-optimization baseline is not included,
because the synthetic framework isolates intra-RIC structural conflict
diagnosis without the full E2 QoS-profiling pipeline; such a
comparison is left to a testbed study.
...
Network-level indicators (throughput, energy per bit, handover
failures, and Jain's fairness index) are recorded for completeness in
Table~\ref{tab:network-results}, but, for the reason given in
Section~\ref{sec:network-perf}, they are reported only to document
that all feasible methods behave comparably, not as a basis for
ranking the methods.
...
$30$ independent seeds. Paired comparisons use identical network
traces and seeds.

For binary outcomes, the analysis reports paired proportions with
$95\%$ confidence half-widths and Holm-corrected $p$-values against
the continuous-only coordinator (Table~\ref{tab:main-results}). For
the latency and repair-core distributions, it reports medians and
upper percentiles rather than means, because these distributions are
heavy-tailed (Tables~\ref{tab:main-results} and~\ref{tab:repair}).
Timeouts are treated as right-censored observations for runtime plots
and are not silently converted into ordinary measured latencies; in
the reported experiment no repair timeout occurred
...
Before interpreting the main results, we verify the soundness
property of Proposition~\ref{prop:soundness} empirically. Across all
$750$ evaluated decision epochs of the main experiment (five
scenarios, five methods, thirty seeds), no action was ever forwarded
as feasible without passing the independent verifier, and no repaired
action was accepted while violating a registered hard relation.
...
Figures~\ref{fig:feasibility}--\ref{fig:latency} report the main
results. A decision is counted as feasible only when the independent
verifier accepts it.
...
Table~\ref{tab:main-results}. First, the difference between
continuous-only coordination (74.7\%) and \method\ (100\%) is
entirely attributable to exact local repair: the two share the same
continuous stage and stall rate (25.3\%), and repair converts every
stalled epoch into a verified feasible decision. Second, \method\
matches the feasibility of complete CP-SAT (100\%), but does
\emph{not} reduce latency: on these instances CP-SAT is both complete
and fast (median $6.0$~ms), whereas the continuous stage dominates the
\method\ latency (median $41.8$~ms). We state this plainly because it
is the honest headline: on instances of the tested size, a
practitioner whose only goal is a feasible decision should simply run
CP-SAT, and the value of \method\ lies in the explicit stall
diagnosis and the independent verification guarantee rather than in
speed. Third, the uncoordinated and static-priority baselines are far
from feasible (0\% and 10\%), confirming that unmanaged or purely
greedy arbitration is inadequate under tight, symmetric conflict
relations.
...
\begin{table}[t]
\centering
\caption{Operational fractional-stall diagnostics by scenario}
\label{tab:stalls}
\begin{tabular}{lrrr}
\toprule
Scenario &
...
\method\ increases from $22$ to $241$~ms because of its continuous
stage, whereas complete CP-SAT stays below $10$~ms; the
continuous-only stall rate remains substantial (5--40\%). \method\
retains $100\%$ verified feasibility at every size.}
\label{fig:scalability}
\end{figure}

The stalls reported in Table~\ref{tab:stalls} are operational
fractional stalls in the sense of Definition~1, not exact fixed
points; the two are distinguished throughout, and only the motif of
Section~\ref{sec:analysis} is proven to admit an exact fractional
fixed point. The analysis includes a scatter plot of
$\rho(\bm{p})$ against $\phi(\bm{p})$, with points separated into
valid decoded actions, invalid decoded actions, repaired actions, and
fallback actions.

Figure~\ref{fig:scalability} illustrates the scalability of the
approach with problem size. The continuous stage is the dominant and
fastest-growing cost, which is the principal scalability limit of
\method\ relative to a complete solver on instances of this size.
This analysis is essential: a plateau in the violation count alone
must not be presented as proof that the continuous state is a fixed
point. Whether the continuous stage ever becomes advantageous
requires a regime in which complete solving is no longer cheap; on the
instances tested here it is not, and we do not claim otherwise.

\subsection{Network-level performance}
\label{sec:network-perf}

\begin{table*}[t]
\centering
\caption{Network-level indicators, averaged over the main experiment.
Throughput is the mean per-user throughput; energy per bit is
expressed in microjoules; the handover-failure rate and Jain fairness
index are derived from the pseudo-physical network model. Because
action utilities are drawn independently of these KPMs, the table is
reported only to document that all feasible methods behave comparably,
not as a basis for ranking.}
...
Because the action utilities are drawn independently of the physical
KPMs, this simulator is by construction insensitive to the choice of
coordination method at the network level: all feasible methods deliver
comparable throughput ($11.3$--$12.0$~Mbps), energy efficiency,
handover reliability, and fairness, and the small residual differences
in Table~\ref{tab:network-results} are not attributable to
coordination quality. The relevant distinction in this study is
therefore feasibility and verification, not raw network utility.
Establishing a genuine network-utility effect requires either coupling
utilities to the KPM model or a hardware-in-the-loop testbed, and is
left to future work; we deliberately omit a normalized-KPI bar chart
here to avoid inviting an unwarranted performance ranking.

\subsection{Repair-core size and latency}

The central scalability hypothesis is that exact repair is practical
when conflicts are localized. Consequently, the evaluation reports the
full distribution of repair-core sizes.
...
To isolate the contribution of each algorithmic component, we
evaluate the following configurations on the stall-prone S4 regime
over $25$ seeds:

\begin{enumerate}
    \item full \method\ (polarization $\beta=2$, escalating exact
    repair);
    \item no polarization ($\beta=1$);
    \item no exact repair (decode only);
    \item repair radius $h=0$ (violated-constraint core only, then
    full);
...
\caption{Ablation study demonstrating the contribution of each
algorithmic component to the final feasible rate.}
\label{fig:ablation}
\end{figure}

Two effects stand out. Removing exact repair collapses the verified
feasible rate from $100\%$ to $48\%$, which is precisely the fraction
of epochs whose continuous decode was already valid; repair is
therefore the component responsible for the remaining feasibility.
Removing polarization ($\beta=1$) does not change the final
feasibility --- escalating repair still resolves every conflict ---
but it multiplies the P95 latency from $83$ to $563$~ms, because a
sharper continuous stage resolves more conflicts locally and avoids
costly full-scope repairs. This is the most consequential algorithmic
finding of the study: polarization is what keeps tail latency
bounded, even though it does not affect final feasibility. Increasing
the repair radius beyond $h=1$ yields no additional feasibility while
slightly enlarging the core, confirming that conflicts are localized
(median repair core of two variables).
...
analytically, as it is only for the motif of
...
First, and most importantly for the scope of the claims, the entire
evaluation is conducted on synthetic planted-feasible instances whose
stall rate is a direct function of the fraction of symmetric
separation relations in the generator. The reported stall rate is
therefore a property of the generator, not a measurement of realistic
O-RAN traffic; the results should be read as a controlled study of a
failure mode, not as an estimate of how often that mode occurs in
deployment.

Second, on instances of the tested size the continuous stage adds
latency relative to a complete CP-SAT solver rather than reducing it.
The hypothesis that the continuous stage becomes advantageous once
complete solving is no longer cheap is not established here and remains
open.

Third, implicit conflicts cannot be verified solely from E2 control
messages. Their identification depends on the quality of the
registered parameter--KPM model, simulator, digital twin, or operator
rule.

Fourth, the continuous update is heuristic. No global convergence or
approximation guarantee is claimed.

Fifth, exact local repair is complete only relative to its induced
subproblem and time budget. Fixing variables outside the repair core
may exclude a feasible global solution.

Sixth, the verifier guarantees consistency with the normalized
conflict model, not semantic equivalence between that model and the
physical RAN.

Finally, simulator-based results may not capture all implementation
delays, message losses, hardware effects, or vendor-specific
behaviour. A hardware or emulated testbed should therefore be used for
at least one representative validation scenario, and all headline
results should be stratified by conflict type, network load, and xApp
count.
...
still produces a conflicting action: an exact fractional fixed point
exists in the smallest arbitration motif, and its operational
counterpart is detected in larger instances. Instead of inserting
overlapping uncorrected factors, the framework extracts the
conflict-induced subproblem and delegates it to an exact solver under
a strict time budget.

The evaluation demonstrates that continuous-only coordination stalls
on roughly one quarter of decision epochs and reaches only $74.7\%$
verified feasibility, whereas both complete CP-SAT and \method\ reach
$100\%$ verified feasibility. Exact repair is the decisive component
of \method: without it, feasibility falls to the $48\%$ of epochs
whose continuous decode was already valid. An important and honest
finding is that, on synthetic instances of this size, the complete
CP-SAT solver is itself fast (median $6$~ms) and complete, so the
continuous stage of \method\ increases decision latency (median
$42$~ms) rather than reducing it. The practical value of \method\ on
these instances is therefore the explicit diagnosis of fractional
stalls and the independent verification guarantee, not raw speed; its
median repair core of two variables confirms that conflicts are
localized, and repair radii $h \geq 2$ yield diminishing returns.
Whether the continuous stage becomes advantageous at larger scales,
where complete solving is no longer cheap, remains the central open
question and the primary motivation for the planned
...
across multiple near-real-time RICs, a network model whose utilities
are coupled to the KPMs so that RQ on network utility becomes
answerable, and hardware-in-the-loop validation.
...
\textbf{Lillia Ouali:} conceptualization, methodology, software,
investigation, validation, writing---original draft.
\textbf{Madani Bezoui:} conceptualization, methodology, formal
analysis, software, validation, visualization, writing---review and
editing, supervision.
\textbf{Kamal Amroun:} methodology, validation, writing---review and
editing.
\textbf{Ahcene Bounceur:} methodology, validation, supervision,
writing---review and editing.
All authors read and approved the final manuscript.

\subsection*{Data availability}

The code and audited raw logs are available in a Zenodo repository.
The DOI will be inserted upon acceptance; during peer review the
artifact is provided as anonymized supplementary material.

\subsection*{Code availability}

The artifact manifest and verifiable execution trace are provided in
the Zenodo repository referenced above.
...
A generative language model was used to assist in the initial
organization and drafting of the manuscript. The authors reviewed,
corrected, and validated the resulting text and assume full
